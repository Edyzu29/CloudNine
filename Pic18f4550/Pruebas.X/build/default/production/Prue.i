...
uint8_t Detecta_Cambio(void){
    ...
    if (((diferencia1>10)||(diferencia1<-10))&&((diferencia2>10)||(diferencia2<-10))){
        salida = 1;
    }else{
        salida = 0;
    }
...
void __attribute__((picinterrupt("low_priority"))) ADC_ISR(void){
    if(PIR1bits.ADIF==1){
        LATDbits.LATD1=1;
        bandera_leer_adc = 1;
        PIR1bits.ADIF=0;
